...
class NullDGP:
    ...
    det_coefs : numpy.ndarray, shape (m,), optional
        Coefficients on caller-supplied deterministic columns (Fourier
        terms, for the unified framework). ``None`` when there are none.
        The columns themselves are a function of time, so the *values*
        are provided again at simulation, extended through the burn-in
        the same way the trend is.
    x_const : numpy.ndarray, shape (k_marginal,)
        Intercepts of the marginal VAR — the drift of the regressors.

        ``k_marginal`` is the number of regressors the marginal model
        generates, which is **not** always the number the conditional
        equation carries. Under a NARDL null the marginal VAR runs on
        the original series, because the partial sums are a
        deterministic function of them: their increments are sign
        constrained, so a VAR fitted to the pieces would generate paths
        that are not partial sums of anything. The conditional equation
        then carries the decomposed columns, and :attr:`q` has one entry
        per decomposed column.
    ...
    det_coefs: FloatArray | None = None
    ...
    def n_regressors(self) -> int:
        """Regressors the marginal model generates."""
        return self.x_const.size

    @property
    def n_conditional(self) -> int:
        """Regressor columns the conditional equation carries.

        Equal to :attr:`n_regressors` unless the conditional equation was
        built on a transform of them.
        """
        return len(self.q)
...
def estimate_null_dgp(
    ...
    var_order: int = 1,
    conditional: bool = True,
    x_marginal: FloatArray | None = None,
    det: FloatArray | None = None,
) -> NullDGP:
    ...
    conditional : bool, default True
        ...
        tested.
    x_marginal : numpy.ndarray, shape (T, k_marginal), optional
        Series the marginal VAR is fitted to, when they differ from the
        columns ``x`` of the conditional equation. The case this exists
        for is the NARDL null: ``x`` holds the partial sums, whose
        increments are sign constrained and cannot be generated by a
        VAR, while ``x_marginal`` holds the original series, which can.
        The decomposition is then re-applied to each regenerated path.
        Defaults to ``x``.
    det : numpy.ndarray, shape (T, m), optional
        Extra deterministic columns of the conditional equation, on the
        dates of the observed sample. They stay in the null model — a
        deterministic term does not vanish when the level relationship
        does — and their coefficients are stored so the regenerated
        paths carry the same component.
    ...
    if det is not None:
        ...
        if det_arr.ndim != 2 or det_arr.shape[0] != n:
            raise ValueError(
                f"det must be 2-D with {n} rows, got shape {det_arr.shape}."
            )
        n_det = det_arr.shape[1]
        for j in range(n_det):
            cols.append(det_arr[start:, j])
    ...
    pos += int(has_trend)
    det_coefs = (
        np.asarray(coefs[pos : pos + n_det], dtype=np.float64) if n_det else None
    )
    pos += n_det
    ...
    dx_marginal = dx if x_marginal is None else np.diff(np.asarray(x_marginal), axis=0)
    if dx_marginal.shape[0] != dx.shape[0]:
        raise ValueError(
            "x_marginal must have the same number of observations as x: "
            f"got {dx_marginal.shape[0] + 1} against {dx.shape[0] + 1}."
        )
    x_const, x_ar, eta = _fit_marginal_var(
        np.asarray(dx_marginal, dtype=np.float64), var_order
    )
...
def simulate_paths(
    ...
    expand : callable, optional
        Maps one period of generated increments, shape ``(B,
        k_marginal)``, to the increments the conditional equation
        carries, shape ``(B, k_conditional)``. It must be **pointwise in
        time**: the transform sees one date at a time, which is what
        makes it composable with a recursion. The NARDL decomposition
        qualifies — the increments of a partial sum are a pointwise
        function of the increments of the series. Defaults to the
        identity.
    det_paths : numpy.ndarray, shape (burn_in + T, m), optional
    ...
    x_star : numpy.ndarray, shape (B, T, k_marginal)
        The regenerated regressors on their **original** scale. A caller
        that passed ``expand`` re-applies its own transform to these,
        rather than receiving transformed levels here: the levels of a
        partial sum depend on a starting convention that belongs to the
        decomposition, not to the simulation.
    ...
    if (dgp.det_coefs is not None) != (det_paths is not None):
        raise ValueError(
            "det_paths must be supplied exactly when the DGP carries "
            "deterministic columns."
        )
    if det_paths is not None and dgp.det_coefs is not None:
        det_arr = np.asarray(det_paths, dtype=np.float64)
        if det_arr.shape != (n_total, dgp.det_coefs.size):
            raise ValueError(
                f"det_paths must have shape {(n_total, dgp.det_coefs.size)}, "
                f"got {det_arr.shape}."
            )
        det_contrib = det_arr @ dgp.det_coefs
    else:
        det_contrib = None
    ...
    # runs exactly the arithmetic it ran before.
    dz = dx if expand is None else np.zeros((n_rep, n_total, k_cond))
    ...
    for t in range(lag_max, n_total):
        ...
        if det_contrib is not None:
            val += det_contrib[t]
        for i in range(1, dgp.p):
            val += dgp.psi[i - 1] * dy[:, t - i]
        for j in range(k_cond):
            for i in range(dgp.first_lag, dgp.q[j]):
                val += dgp.omega[j][i - dgp.first_lag] * dz[:, t - i, j]
        dy[:, t] = val + inn[:, t, 0]
